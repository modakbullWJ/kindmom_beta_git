...
//5-1. 선택한 그룹별로 원하는 수만큼 보여줌 조회수 순서로 출력 추가 WJ
//   사용법 : <?php echo latest_group("최신글스킨", "그룹이름", 게시물수, 제목글자수, 본문글자수);? >
//출처: http://sir.co.kr/bbs/board.php?bo_table=g4_skin&wr_id=95895

function latest_group_hit_img($skin_dir="", $gr_id, $rows=10, $subject_len=40, $contents_len=200, $category="", $orderby="") {
  global $config;
  global $g5;
  ...
  for ($j=0, $k=0; $rowgroup=sql_fetch_array($rsgroup); $j++) {
    ...
    // 옵션에 따라 정렬
    $sql = "select * from $tmp_write_table where wr_is_comment = 0  and wr_file IN ('1','2') ";
    $sql .= (!$category) ? "" : " and ca_name = '$category' ";
    $sql .= (!$orderby) ? "  order by wr_id desc " : "  order by $orderby desc, wr_id desc ";
    $sql .= " limit $limitrows";
    //echo $sql;
    ...
    for ($i=0; $row = sql_fetch_array($result); $i++, $k++) {
      ...
      if(!$orderby) $op_list[$k] = $row[wr_hit];
      else  {
        $op_list[$k] = is_string($row[$orderby]) ? sprintf("%-256s", $row[$orderby]) : sprintf("%016d", $row[$orderby]);
        $op_list[$k] .= $row[wr_hit];
        $op_list[$k] .= $row[wr_name];
        $op_list[$k] .= $row[wr_10];
      }
